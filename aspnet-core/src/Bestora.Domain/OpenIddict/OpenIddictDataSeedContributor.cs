...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Bestora") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Bestora", DisplayName = "Bestora API", Resources = { "Bestora" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["Bestora_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrls = configurationSection["Bestora_App:RootUrl"]
            ...
            await CreateApplicationAsync(
                ...
                clientUri: consoleAndAngularClientRootUrls.FirstOrDefault(),
                postLogoutRedirectUri: consoleAndAngularClientRootUrls.FirstOrDefault()
            );
        }
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Bestora_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Bestora_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        List<string>? permissions = null)
    {
        ...
        if (redirectUris != null)
        {
            foreach (var uriString in redirectUris)
            {
                if (!string.IsNullOrWhiteSpace(uriString))
                {
                    ...
                    {
                        throw new BusinessException(L["InvalidRedirectUri", uriString]);
                    }
        ...
        // Handle postLogoutRedirectUri as before
        ...
        // Update redirectUris if different
